    ...
    <div class="look-input" v-show="isnullShow">
      ...
      <div class="look-input-bot" :style="{height:isTextHeight}">
        <div class="look-input-bot-input">
          <textarea maxlength="5000" v-model.trim="texteCommentsVal" ref="pickerdate" :style="{height:isTextHeight,'lineHeight':isTextlineHeight}"
...
export default {
  ...
  methods: {
    ...
    listFun() {
      var self = this;
      if (sessionStorage.getItem("logListsIndexObj")) {
        self.isnullShow = true;
        self.$store.commit("setMutVuexisLoading", true);
        self.listObj = JSON.parse(sessionStorage.getItem("logListsIndexObj"));
        document.title =
    ...
    dataFun() {
      var self = this;
      // 设置已读
      if (self.listObj.user.id != sessionStorage.getItem("logMemberId")) {
        self
          .getlogsearchread(
            JSON.stringify({
              logId: self.listObj.logId,
    ...
    ...mapActions(["getapicomments", "getapicomment", "getlogsearchread"])
  },
  created: function() {
    var self = this;
    self.listFun();
