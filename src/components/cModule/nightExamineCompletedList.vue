...
export default {
  ...
  updated: function() {
    this.$nextTick(function() {
      let position = this.$store.state.VuexCompletedApprovalTop || 0; //返回页面取出来
  ...
  methods: {
    ...
    nightExamineClick(index) {
      ...
      self.$store.commit("setMutVuexCompletedApprovalClickIndex", index);
    },
    // 下拉刷新
    onRefresh(done) {
      let self = this;
            ...
            if (data.statusCode == "200") {
              if (data.data.rows) {
                if (data.data.rows.length > 0) {
                  self.$emit("onRefreshCompletedFun", "111");
                  self.nightlist = data.data.rows;
                  self.pageNoVal++;
                  self.$store.commit(
                    "setMutVuexCompletedApprovalLists",
                    self.nightlist
                  );
                  self.$store.commit(
                    "setMutVuexCompletedApprovalIndex",
                    self.pageNoVal
                  );
                }
              }
    ...
    onInfinite(done) {
      ...
      if (self.onInfiniteTrue == false) {
        ...
        if (sessionStorage.getItem("await")) {
          ...
          self.state = awaitObj.genderVal;
          self.deptId = awaitObj.departmentVal;
        }
        if (sessionStorage.getItem("completedSaerch")) {
          var waitedSaerchStr = JSON.parse(
            sessionStorage.getItem("completedSaerch")
          ); //筛选的条件
          self.searchVal = waitedSaerchStr;
        }
                ...
                if (data.data.rows) {
                  if (data.data.rows.length > 0) {
                    var VuexStartedListsTopNum =
                      this.$store.state.VuexCompletedApprovalTop + 56;
                    this.$store.commit(
                      "setMutVuexCompletedApprovalTop",
    ...
    ...mapActions(["getapprovalCompletedApproval"])
  },
  watch: {
    lists: {
      handler(val) {
        var self = this;
        self.nightlist = val;
        self.$store.commit("setMutVuexCompletedApprovalLists", self.nightlist);
        if (self.$store.state.VuexCompletedApprovalIndex != 9999) {
          self.pageNoVal = self.$store.state.VuexCompletedApprovalIndex;
        } else {
          self.pageNoVal = 2;
        }
      },
