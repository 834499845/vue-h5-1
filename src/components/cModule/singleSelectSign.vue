...
export default {
  ...
  methods: {
    listArrFun() {
      ...
      if (self.defIndex * self.topLinst - self.topLinst >= this.numLists.length * self.topLinst - self.topLinst) {
        self.istranslate3d =
          "translate3d(0, -" + (self.numLists.length * self.topLinst - self.topLinst) + "px, 0)";
        self.iswebkitTransform =
          "translate3d(0, -" + (self.numLists.length * self.topLinst - self.topLinst) + "px, 0)";
        self.temps = self.numLists.length * self.topLinst - self.topLinst;
      } else {
        self.istranslate3d = "translate3d(0, -" + self.defIndex * self.topLinst + "px, 0)";
        self.iswebkitTransform =
    ...
    cancelClick(event) {
      ...
      this.$emit("cancel", self.numLists[self.temps / self.topLinst]);
      if (self.defIndex * self.topLinst - self.topLinst >= this.numLists.length * self.topLinst - self.topLinst) {
        self.istranslate3d =
          "translate3d(0, -" + (self.numLists.length * self.topLinst - self.topLinst) + "px, 0)";
        self.iswebkitTransform =
    ...
    confirmClick(event) {
      var self = this;
      event.preventDefault();
      this.$emit("confirm", self.numLists[self.temps / self.topLinst], self.defState);
      if (self.defIndex * self.topLinst - self.topLinst >= this.numLists.length * self.topLinst - self.topLinst) {
        self.istranslate3d =
          "translate3d(0, -" + (self.numLists.length * self.topLinst - self.topLinst) + "px, 0)";
        self.iswebkitTransform =
    ...
    pockerShowTouchmove(event) {
      ...
      if (self.temps == self.maxHeight || self.temps == 0) {
        ...
        if (self.temps == 0 && moveNum < 0) {
          self.istranslate3d = "translate3d(0, " + 2.5 * self.topLinst + "px, 0)";
          self.iswebkitTransform = "translate3d(0, " + 2.5 * self.topLinst + "px, 0)";
        } else {
          self.istranslate3d =
