            ...
            <div class="logLi-top" @click="logListsClick(index)">
              <div class="logLi-top-left">
                ...
                <div class="ll-t-lf-right">
                  <div v-if="item.user" class="ll-t-lf-right-name">
                    {{item.user.realName}}
                  </div>
            ...
            <div class="allLog-text">
              ...
              <div class="allLText-list" v-show="item.logImages">
                ...
                <div class="aLT-list-img">
                  <ul>
                    <li class="aLT-list-imgLi" @click="showImgListsClick(index)" v-for="(imgItem,imgIndex) in item.logImages" :key="imgIndex">
...
export default {
  ...
  watch: {
    lists: {
      handler(val) {
        var self = this;
        self.logLists = val;
        self.$store.commit("MutVuexUnreadLogLists", self.logLists);
        if (self.VuexUnreadLogListsIndex != 9999) {
          self.pageNoVal = self.VuexUnreadLogListsIndex;
        } else {
          self.pageNoVal = 2;
        }
      },
  ...
  methods: {
    ...
    logListsClick(index) {
      ...
      var VuexUnreadLogListsArr = self.VuexUnreadLogLists;
      VuexUnreadLogListsArr.splice(index, 1);
      self.$store.commit("MutVuexUnreadLogLists", VuexUnreadLogListsArr);
    },
    // 下拉刷新
    onRefresh(done) {
      var self = this;
            ...
            if (data.statusCode == 200) {
              if (data.data) {
                if (data.data.records) {
                  ...
                  self.$store.commit(
                    "MutVuexUnreadLogListsIndex",
                    self.pageNoVal
                  );
                  self.$store.commit("MutVuexUnreadLogLists", self.logLists);
                  self.$emit("onRefreshList", 2);
                } else {
                  self.$emit("onRefreshList", 1);
                }
              } else {
                self.$emit("onRefreshList", 1);
              }
