  ...
  <div>
    <div class="MedDatedatetimePicker" v-show="dataTrue">
      <div @click="cancelClick"></div>
        <div>
          <mt-datetime-picker @cancel="cancelClick" @confirm="confirmClick" year-format="{value}" month-format="{value}" date-format="{value}"  hour-format="{value}"  minute-format="{value}"
            ref="pickerdate" type="datetime" v-model="dataVal">
          </mt-datetime-picker>
        </div>
    </div>
  </div>
