  ...
  <div class="yo-scroll" @touchstart="touchStart($event)" @touchmove="touchMove($event)" @touchend="touchEnd($event)" @scroll="(onInfinite || infiniteLoading) ? onScroll($event) : undefined">
    <section class="inner" :style="{ 'webkitTransform':'translate3d(0, ' + top + 'px, 0)',transform: 'translate3d(0, ' + top + 'px, 0)','transition':'transform 0ms ease-out' }">
      ...
      <footer class="load-more">
        <span v-show="loadMoreShow">
          <mt-spinner type="snake" color="#4390E1"></mt-spinner>
        </span>
        <slot name="load-more">
          <span class="down-text">{{loadingText}}</span>
        </slot>
      </footer>
    </section>
  </div>
...
export default {
  ...
  data() {
    return {
      ...
      startY: 0,
      touching: false,
      isRefreshDoneShow: false,
      isRefreshScroll: false,
      infiniteLoading: false // 上拉加载状态控制
    };
  },
  methods: {
    touchStart(e) {
    ...
    touchEnd(e) {
      ...
      if (this.top >= this.offset) {
        // do refresh
        if (this.isRefreshDoneShow == false) {
          this.refresh();
        }
      } else {
        // cancel refresh
...
.refreshScroll {
  ...
  width: 100vw;
  height: 100vh;
  background-color: rgba(44, 4, 4, 0);
  left: 0;
}
