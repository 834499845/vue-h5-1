<template xmlns:v-on="http://www.w3.org/1999/xhtml">
  ...
  <div class="newsVideo">
    ...
    <div v-show="!pathShow" class="newsVideoContent">
      ...
      <!-- src="" -->
      <div class="videoCssF">
        <video id="my-video" x5-video-player-type="h5" x5-video-player-fullscreen="false" class="video-js" controls preload="auto" width="320" height="200" :poster="posterStr">
          <source src="" type='video/mp4'>
          <!-- <source :src="videoObj.content" type='video/webm'> -->
          <p class="vjs-no-js">
...
export default {
  ...
  mounted() {
    ...
    // myPlayer.src('http://192.168.10.34:1111/BOTU/news/video/20180203/635a91c7d081445480d25edddac0795b.mp4');  //重置video的src
    // myPlayer.load('http://192.168.10.34:1111/BOTU/news/video/20180203/635a91c7d081445480d25edddac0795b.mp4');  //使video重新加载
  },
  created: function() {
    var self = this;
    ...
    // f1afa7f129d64d7abb3b2a638178c697 043415c8e57a44d4831c9471c51f1134
    if (idStr) {
      self
        ...
        .then(data => {
          if (data.code == 200) {
            if (data.data) {
              ...
              // self.posterStr = self.videoObj.cover;
              // var myPlayer = new videojs("my-video");
              // myPlayer.src(self.videoObj.content); //重置video的src
              // myPlayer.load(self.videoObj.content); //使video重新加载

              var videoPlayer = videojs("my-video", {}, function() {
                //手动设置视频播放器的视频源
                // player.src(self.videoObj.content);
              });
              videoPlayer.src(self.videoObj.content);
              self
                .getcommentsInfopage({
                  current: 1,
