...
<template>
    <div>
        <div class="detailsExamine" v-show='codeShow'>
            ...
            <div class="approver" v-if="copyList.length">
            ...
            <div v-show="state == '0'" class="revocation" @click="revocationClick()">
                撤销
            </div>
        </div>
        <div class="codeNo" v-show='!codeShow'>
    ...
    export default {
        ...
        data() {
            return {
                ...
                position: "", //职位
                codeShow: false,
                cancelUpdateShow: false //询问框
            };
        },
        created: function () {
            var self = this;
            sessionStorage.setItem("isDetailsExamineLook", 1);
            document.title = "我发起的";
            if (sessionStorage.getItem("leavrShow")) {
                sessionStorage.removeItem("leavrShow");
            }
        ...
        methods: {
            //撤销
            revocationClick() {
                this.cancelUpdateShow = true;
            },
            // 询问框确认撤销
            cancelUpdateaffrimClick() {
                this.cancelUpdateShow = false;
                var self = this;
                self.getapprovalRevoke({
                    companyId: sessionStorage.getItem("companyId"),
                    memberId: sessionStorage.getItem("memberId"),
                    approvalId: self.$route.query.approvalId
                }).then(data => {
                    if (data.statusCode == 200) {
                        if (sessionStorage.getItem("sponsor")) {
                            var WaitedApprovalLists = self.$store.state.VuexStartedLists;
                            var nightlistStrIndex = self.$store.state.VuexStartedClickIndex;
                            WaitedApprovalLists.splice(nightlistStrIndex, 1);
                            self.$store.commit("setMutVuexStartedLists", WaitedApprovalLists);
                        } else {
                            var WaitedApprovalLists = self.$store.state.VuexStartedLists;
                            var nightlistStrIndex = self.$store.state.VuexStartedClickIndex;
                            WaitedApprovalLists[nightlistStrIndex].message = "已撤销";
                            WaitedApprovalLists[nightlistStrIndex].state = 2;
                            self.$store.commit("setMutVuexStartedLists", WaitedApprovalLists);
                        }
            ...
            ...mapActions(["getapprovalApprovalDetail", "getapprovalRevoke"])
        },
